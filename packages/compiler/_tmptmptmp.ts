import * as html from './public_api';
...
export function humanizeLineColumn(location: html.ParseLocation): string {
...
const parser = new html.HtmlParser();

const {errors, rootNodes} = parser.parse('@foo {<strong>hello}', 'TestComp');

console.log(errors);

for (const node of humanizeNodes(rootNodes)) {
  console.log(node);
}
